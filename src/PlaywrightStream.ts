...
export async function launch(
	...
	if (opts.headless) {
        ...
        const userDataDir = opts.userDataDir || fs.mkdtempSync(path.join(os.tmpdir(), "pw-stream-"));
        ...
        (await context.newPage()).goto(`chrome-extension://${extensionId}/options.html#${port}`);

        const old_browser_close = context.close.bind(context);
        context.close = async () => {
                ...
                const extension = await getExtensionPage(context);
...
export async function getExtensionPage(context: BrowserContext) {
        for (const page of context.pages()) {
                if (page.url().startsWith(`chrome-extension://${extensionId}/options.html`)) return page;
        }
...
export async function getStream(page: Page, opts: getStreamOptions) {
	...
	const retryPolicy = Object.assign({}, { each: 20, times: 3 }, opts.retry);
        ...
        const extension = await getExtensionPage(page.context());
	...
	function onConnection(ws: WebSocket, req: IncomingMessage) {
		...
		async function close() {
			if (!stream.readableEnded && !stream.writableEnded) stream.end();
                        if (!extension.isClosed() && extension.context().browser()?.isConnected()) {
				// @ts-ignore
